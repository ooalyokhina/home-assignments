...
def _build_impl(frame_sequence: pims.FramesSequence,
    ...
    for frame, cur in enumerate(frame_sequence[1:]):
        ...
        pts, status, err = cv2.calcOpticalFlowPyrLK(prev, cur, pts, None, winSize=(dist, dist), minEigThreshold=0.002)
        pts = pts[status.reshape(-1).astype(np.bool)]
        ids = ids[status.reshape(-1).astype(np.bool)]
        if len(pts) < 1000:
            mask = get_mask(cur, pts, dist)
            params['maxCorners'] = 1000 - len(pts)
        ...
        builder.set_corners_at_frame(frame + 1, FrameCorners(ids, pts, np.full(len(pts), dist)))
        prev = cur
